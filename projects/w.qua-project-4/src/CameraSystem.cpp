...
void CameraSystem::ProcessKeyboardInput(float deltaTime)
{
    ...
    if (camera.m_ActiveCameraType == CameraType::FPS) 
    {
        ...
        bool shiftHeld = Systems::GetInputSystem()->IsKeyPressed(Keybinds::KEY_LEFT_SHIFT) ||
                          Systems::GetInputSystem()->IsKeyPressed(Keybinds::KEY_RIGHT_SHIFT);

        float speedMultiplier = shiftHeld ? 3.0f : 1.0f;
        float velocity = camera.m_FPS.m_MovementSpeed * speedMultiplier * deltaTime;
    ...
    else if (camera.m_ActiveCameraType == CameraType::Orbital) 
    {
        ...
        bool shiftHeld = Systems::GetInputSystem()->IsKeyPressed(Keybinds::KEY_LEFT_SHIFT) ||
                          Systems::GetInputSystem()->IsKeyPressed(Keybinds::KEY_RIGHT_SHIFT);
        float speedMultiplier = shiftHeld ? 3.0f : 1.0f;
        float targetSpeed = 2.0f * speedMultiplier * deltaTime;
